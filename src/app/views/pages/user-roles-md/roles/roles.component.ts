...
export class ChecklistDatabase {
	...
	constructor(private service: AuthService) {
	 ...
	 initialize() {
		...
		.subscribe(permissions =>{	const data = this.buildFileTree(permissions, 0)
...
export class RolesComponent implements OnInit {
  ...
  checklistSelection3 = new SelectionModel<TodoItemFlatNode>(true /* multiple */);
	...
	get role() { return this.roleForm?this.roleForm.controls:null; }
	get permission() { return this.role?this.role.permissions as FormArray:null; }
  ...
  hasNoContent3 = (_: number, _nodeData: TodoItemFlatNode) => _nodeData.item === '';
	...
	descendantsPartiallySelected3(node: TodoItemFlatNode): boolean {
		...
		if(result)
		this.checklistSelection3.select(node)
		else
		this.checklistSelection3.deselect(node)
