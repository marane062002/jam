...
export class PrestataireEditComponent implements OnInit {
  ...
  constructor( 
      ...
      console.log('ID: ' + this.id);
		  this.prestataireForm = this.formBuild.group({
      id: [this.id],
    ...
    this.addForm2 = this.formBuild.group({
      _file: []
    });
       
      this.service.
      getPrestataireById(this.id)
      .then(data =>{
       console.log('Show data : '+JSON.stringify(data,null,4));
       this.prestataireForm.patchValue(data);
      })
  ...
  onSubmit(){
    ...
    this.loading = true;
      const prestataire: any = Object.assign({}, formValues);
       ...
       this.service.updatePrestataire(prestataire,this.id)
      .subscribe(data =>{
