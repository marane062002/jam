...
export class ListTypeIndicateurDelegataireComponent implements OnInit {
	...
	loadData(page: number, pageSize: number): void {
		...
		this.httpClient.get<any[]>(`${this.baseUrl}type-indicateur/paginate/${page}/${pageSize}`,{ headers: this.headers } ).subscribe((response: any) => {
	...
	addTypeIndicDelegataire(): void {
		this.router.navigate(['parametrage-sdl/add-type-indicateur-delegataire']);
	}
	updateTypeIndicDelegataire(id: number): void {
		this.router.navigate(['parametrage-sdl/upd-type-indicateur-delegataire/' +id]);
	}
	...
	deleteDelegataire(id): void {
		this.httpClient.delete(`${this.baseUrl}type-indicateur/${id}`,{ headers: this.headers }).subscribe(
			(response) => {
