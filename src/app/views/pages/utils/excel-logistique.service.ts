...
export class ExcelLogistiqueService {
	...
	public exportAsExcelFile(reportHeading: string, reportSubHeading: string, headersArray: any[], json: any[], footerData: any, excelFileName: string, sheetName: string) {
		...
		worksheet.getCell("C1").value = "Affaires sociales / Associations / Accompagnement et partenariat / Support logistique";
		...
		worksheet.getCell("C2").value = reportHeading;
		...
		if (reportHeading !== "") {
			...
			worksheet.getCell("C3").value = "Date : " + this.datePipe.transform(new Date(), "dd-MM-yyyy");
