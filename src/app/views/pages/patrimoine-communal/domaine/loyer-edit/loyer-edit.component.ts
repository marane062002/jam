...
export class LoyerEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.loyerForm = this.fb.group({      
      ...
      this.service.getLoyerDomaineById(this.id)
      .then(data =>{
  ...
  this.getLignesLoyer()
    ...
    .subscribe(data =>{
      ...
      if(this.uploadFiles) 
            this.service.updloadFilet(this.uploadFiles, m.id,this.type1,'/PjLoyersDomaines/multiplefile')
            .subscribe(resp=>{console.log(resp)},
            error => console.log(error))
     if(this.uploadFiles2)
      this.service.updloadFilet(this.uploadFiles2, m.id,this.type2,'/PjLoyersDomaines/multiplefile')
      .subscribe(resp=>{console.log(resp)},
      error => console.log(error))
      this.router.navigate(['domaine/domaine-show'], { queryParams: { id: loyer.domaine.id } })
              
               },
              error => console.log(error)
            );

  //  this.service.saveLoyerDomaine(loyer)
  //   .subscribe(data =>{
  //     let m:any=Object.assign({}, data);
