...
export class LigneloyerEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    getLloyer(){
      this.service.getLigneLoyerToiletteById(this.id)
      .then(data => {
  ...
  onSubmit(){
   ...
   this.service.updateLigneLoyerToilette(lloyer,this.id)
    .subscribe(data =>{
      let m:any=Object.assign({}, data);
    
      if(this.uploadFiles) 
            this.service.updloadFilet(this.uploadFiles, m.id,this.type1,'/PjLignesLoyerToilette/multiplefile')
            .subscribe(resp=>{console.log(resp)},
            error => console.log(error))
     if(this.uploadFiles2)
      this.service.updloadFilet(this.uploadFiles2, m.id,this.type2,'/PjLignesLoyerToilette/multiplefile')
      .subscribe(resp=>{console.log(resp)},
      error => console.log(error))

              this.router.navigate(['toilette/loyer-show/'], { queryParams: { id: lloyer.loyer.id } }) 
