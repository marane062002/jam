...
@Component({
  selector: 'kt-vali-dg-consultation',
  templateUrl: './vali-dg-consultation.component.html',
  styleUrls: ['./vali-dg-consultation.component.scss']
})
export class ValiDgConsultationComponent implements OnInit {
...
idao:number;
  ...
  onSubmit(value:any){
    ...
    this.aoService.updateVisaPresident(this.idao, value).subscribe(res=>{
      console.log(res);
	  Swal.fire({
		title: 'Visa de president à été bien traité',
		icon: 'success',
	  }
...
/*	// ================================================================================
	...
	ngOnInit() {
		...
		this.service.getAoById(this.idao).subscribe((res) => {
			this.ao = res;
			console.log("AO VDG : " + JSON.stringify(res, null, 2));
		});

		this.fileService.fileSizeDetector();

		this.service
			.getFilesByIdAndSMOdule(this.idao, "Validation DG")
			.subscribe(
				(data) => {
	...
	populate() {
		const _this = this;
		this.service.getAllReserveDg(this.idao).subscribe(
			(data) => {
	...
	onSubmit(form: NgForm) {
		...
		if (confirm(this.translate.instant("PAGES.GENERAL.MSG_VALIDATE"))) {
			...
			this.service.sendReserveDg(this.formDataDg).subscribe((res) => {
				this.allpjs = [];
