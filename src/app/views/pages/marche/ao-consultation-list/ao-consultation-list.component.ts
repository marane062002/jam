...
@Component({
  selector: 'kt-ao-consultation-list',
  templateUrl: './ao-consultation-list.component.html',
  styleUrls: ['./ao-consultation-list.component.scss']
})
export class AoConsultationListComponent implements OnInit {
  ...
  aoData: any;
	...
	async ngOnInit() {
		...
		if (this.isSearch == false) {
				...
				//var spinnerRef = this.spinnerService.start(this.translate.instant("PAGES.GENERAL.LOADING"));
					...
					this.service.findAllByPages().then((data) => {
	...
	onSubmit() {
		...
		if (this.isSearch == true) {
			...
			if (this.formGroup.value.existEchantillon == false) {
	...
	handlePageEvent(event: PageEvent) {
        ...
        this.service.Pagination(pageIndex, pageSize).subscribe(
					(res: any) => {
						console.log(res);
						this.data = res.content;
						this.isLoading = false;
	...
	//=====================================================
	nouvelleao() {
		this.router.navigate(["/marches/ao-consultation-add"]);
	}
	// ====================================================
	//
	//=====================================================
