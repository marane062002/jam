...
@Component({
	selector: "kt-membre-technique-seance",
	templateUrl: "./membre-technique-seance.component.html",
	styleUrls: ["./membre-technique-seance.component.scss"],
})
export class MembreTechniqueSeanceComponent implements OnInit {
	// ====================================================================
	//
	// ====================================================================
	incriPI = 0;
	...
	data_sizeEX
	getDataEx() {
		this.service.getPEbyEtatMembre('MEMBRE_TECHNIQUE',this.idao).then((res) => {
	...
	handlePageEventEX(event: PageEvent) {
		...
		this.service.getPEbyEtatMembrePagination('MEMBRE_TECHNIQUE',this.idao,pageIndex, pageSize).subscribe((res: any) => {
			this.data_sizeEX=res.totalElements
	...
	data_size
	getDataIn() {
		this.service.getPIbyEtatMembre('MEMBRE_TECHNIQUE',this.idao).then((res:any) => {
	...
	handlePageEvent(event: PageEvent) {
		let pageSize = event.pageSize;
		let pageIndex = event.pageIndex;
		this.service.getPIbyEtatMembrePagination('MEMBRE_TECHNIQUE',this.idao,pageIndex, pageSize).subscribe((res: any) => {
			res.content.forEach((item, index) => {
				this.service1.getPersonnelById(res.content[index].idPersonnel).then((data) => {
					item.name = data.nom + " " + data.prenom;
					item.present = res.content[index].present
	...
	onSubmit(form: NgForm) {
		...
		if (this.ParticipantsInternes != null)
			...
			this.ParticipantsInternes.forEach((item, index) => {
				...
				item.etatMembre = 'MEMBRE_TECHNIQUE'
