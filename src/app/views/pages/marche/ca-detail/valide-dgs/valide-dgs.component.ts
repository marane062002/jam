...
@Component({
	selector: "kt-valide-dgs",
	templateUrl: "./valide-dgs.component.html",
	styleUrls: ["./valide-dgs.component.scss"],
})
export class ValideDgsComponent implements OnInit {
	idCA: any;
	noteForm: any;
	...
	valideParDGS() {
		Swal.fire({
			...
			if (result.isConfirmed) {
				let motif =
					this.noteForm.noteDGS != ""
						? this.noteForm.noteDGS
						: "pas d'observation";
				this.service.valideParDGS(this.idCA, motif).subscribe(
					(data) => {
						Swal.fire({
							position: "center",
							icon: "success",
