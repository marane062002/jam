...
import { MatAccordion, MatDialog, MatSelectChange, MatTableDataSource } from "@angular/material";
...
@Component({
	selector: "kt-ao-consultation-add",
	templateUrl: "./ao-consultation-add.component.html",
	styleUrls: ["./ao-consultation-add.component.scss"],
})
export class AoConsultationAddComponent implements OnInit {
	...
	//
	//=====================================================
	constructor(private serviceClassification: GestionClassificationService, private serviceQualification: GestionQualificationService, private serviceTypeAo: GestionDesTypesAoService, private serviceConsultationArchitectural: ConsultationArchitecturalService, private service: AoService, private service2: OrganisationService, private router: Router, private activatedRoute: ActivatedRoute, private fileUtils: FilesUtilsService, private dialog: MatDialog, private translate: TranslateService, private programmeService: ProgrammeService, private conventionMarcheService: ConventionMarcheService) {
	...
	displayedColumns2 = ["Lot", "Objet (Fr)", "caution", "Montant (Dhs)", "actions"];
	typeBien = [
	...
	//=====================================================
	onDateSelected() {
		console.log("Selected date:", this.formData.dateOuverturePlis);
	}
	selectedPAC(event) {
		if (event.value == "1") {
			this.isVisible = true;
	...
	selectedCodeOrientation(event: MatSelectChange) {
		this.projet = "";
		this.listprogramme = [];
		this.programmeService.finAllByCode(event.value).subscribe(
			(res) => {
	...
	selectedCodeProjet(event: MatSelectChange) {
		this.programmeService.findByCodeProjet(event.value).subscribe(
			(res: any) => {
	...
	selectedConsultationArchitecturale(event: MatSelectChange) {
		this.serviceConsultationArchitectural.getConsultationByNumCA(event.value.numCA).subscribe(
			(res: any) => {
				this.objet = res.objetFR;
				this.idConsultationArchitecturale = res.id;
				// this.formData.idProject = res.id;
			},
			(err) => {
				console.log(err);
	...
	existEchantillon() {
		this.formData.existEchantillon = this.isVisible7;
		console.log("existEchantillon", this.formData.existEchantillon);
	}
	existTypeAo() {
		this.formData.existTypeAo = this.isVisible2;
		console.log("existTypeAo", this.formData.existTypeAo);
	}
	existanceAgrement() {
	...
	onSubmit(form: NgForm) {
		// this.formData.agrements=this.selectedAgrementvalue;

		// if (this.formData.modePassation == "Autres") {
		// 	this.formData.modePassation = this.formData.autres;
		// }
		...
		if (this.formData.objet == "" || this.formData.objetAR == "") {
			...
			this.formData.consultationArchitecturale.id = this.idConsultationArchitecturale;
			...
			if (this.formData.consultationArchitecturale.id == "" || this.formData.consultationArchitecturale.id == undefined) {
				this.formData.consultationArchitecturale = null;
			}
			// if (this.formData.existanceAgrement == true) {
			// 	this.selectedAgrementvalue.splice(this.selectedAgrementvalue.length, 1);
			// 	this.formData.agrements = this.selectedAgrementvalue.join(", ");
			// } else {
			// 	this.formData.agrements = null;
			// }
			...
			this.service.sendao(this.formData).subscribe((res) => {
				...
				if (this.allpjs.length > 0) {
					for (var i = 0; i < this.allpjs.length; i++) {
						this.service.nouvellepj(this.allpjs[i].selecetedFile, res["id"], this.allpjs[i].type.id, "AO", this.formPj.LabelPj).subscribe((data) => {
