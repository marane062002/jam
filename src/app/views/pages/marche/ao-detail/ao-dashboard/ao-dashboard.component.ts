...
import { Prestataire } from '../prestataires/prestataires.component';


@Component({
  selector: 'kt-dashboard',
  templateUrl: './ao-dashboard.component.html',
  styleUrls: ['./ao-dashboard.component.scss']
})
export class AoDashboardComponent implements OnInit {
 ...
 // ============================================
	...
	getAllTypeMarche() {
		this.typeMarcheService.getAllTypeMarche()
		.subscribe(
			(data) => {
	...
	onSubmit() {
		...
		console.log(typeMarche +' | '+ statut+' | '+ dateDebut+' | '+dateFin);
		if(dateDebut != ''){
		...
		const _this = this;
		var spinnerRef = this.spinnerService.start(this.translate.instant("PAGES.GENERAL.LOADING")); // start spinner
		this.service.getAoByFilterParameters(dateDebut, dateFin, typeMarche,statut).subscribe(
			data => {
				this.spinnerService.stop(spinnerRef);// stop spinner
				console.log("LISTE MARCHE" + JSON.stringify(data,null,2));
				this.isLoading = false;
