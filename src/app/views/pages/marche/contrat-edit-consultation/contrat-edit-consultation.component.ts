...
@Component({
  selector: 'kt-contrat-edit-consultation',
  templateUrl: './contrat-edit-consultation.component.html',
  styleUrls: ['./contrat-edit-consultation.component.scss']
})
export class ContratEditConsultationComponent implements OnInit {
	contrat = { 
  ...
  ngOnInit() {
    ...
    this.activatedRoute.queryParams.subscribe((params) => {
		...
		this.serviceContrat.getOffresByIdContrat(this.idBonCommande).subscribe((res)=>{
			
			this.list=res
			this.dataSource1 = new MatTableDataSource(this.list);
		})

    this.serviceContrat.getByIdContrat(this.idBonCommande).subscribe((data)=>{
			this.contrat = data
			...
			// 		this.contrat.typeMarche = data.typeMarche;
			...
			// 	}
			// } else {
			// 	this.contrat.typeMarche = { id: "",libelle: ""};
			// }
			// this.contrat.dateDevis = new Date(data.dateDevis).toISOString();
...
// 	}
  ...
  public getAllBCFiles() {
		this.serviceContrat.getByIdContratFiles(this.idBonCommande)
			.pipe(delay(1000))
			.subscribe(
				(data) => {
					this.isLoading = false;
