...
@Component({
  selector: 'kt-contrat-list-consultation',
  templateUrl: './contrat-list-consultation.component.html',
  styleUrls: ['./contrat-list-consultation.component.scss']
})
export class ContratListConsultationComponent implements OnInit {
  ...
  ngOnInit() {
			...
			this.service.getAllContrat().then((data) => {
  ...
  nouvelleao() {
		this.router.navigate(["/marches/contrat-consultation-add"]);
	}
  exportTable() {
	...
	editBonCommand(row) {
		this.router.navigate(["/marches/contrat-consultation-edit"], {
			queryParams: { id: row },
		});
	}
}
