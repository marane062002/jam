...
// 						// this.service.updateao(this.ao).subscribe(result => {});
    ...
    this.service.updateao(this.formData).subscribe(res => {
      ...
      //this.router.navigate(['/personne-physique/personne-physique-list']);
		...
		this.service.getAllLigneBP(this.formDataBP.id)
			...
			.subscribe(
				(data) => {
					console.log("dataLBP"+data)
					_this.dataSize = data.content.length;
					this.dataSourceLBP = new MatTableDataSource(data.content);
					this.isLoading = false;
					// this.dataSourceLBP.paginator = this.paginator;
					// this.dataSourceLBP.sort = this.sort;
				},
	...
	openDialog(): void {
	  ...
	  afterSave(){
		...
		if (this.formDataBP.id == -1) {
				...
				this.formData.bordereauPrix.id = this.formDataBP.id;
				 ...
				 this.dataSourceLBP.data.forEach(item => {
					...
					let lbp={
						...
						tva:item.tva,
						unite:item.unite,
						isForfait:item.isForfait,
						prixEstimati:item.prixEstimati,
						numPrix:item.numPrix,
						prixUnite :item.prixUnite,
						quantite: item.quantite,
						bordereauPrix:{id: this.formDataBP.id}

					}
				this.service.sendLigneBP(lbp).subscribe(r => {
					console.log('LigneBP data sent successfully:', r);
					this.resetFormData();
					this.populate();
				});})
			...
			this.service.sendBP(this.formDataBP).subscribe((res) => {});
			this.dataSourceLBP.data.map(item => {
				let lbp={
					designation : item.objet,
