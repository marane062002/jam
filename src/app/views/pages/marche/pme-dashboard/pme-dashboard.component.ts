...
@Component({
  ...
  templateUrl: './pme-dashboard.component.html',
  styleUrls: ['./pme-dashboard.component.scss']
})
export class PmeDashboardComponent implements OnInit {
 ...
 // ============================================
	...
	// ============================================
	ngAfterViewInit() {
		this.service.getAllTypeAoOrderedByTypeAo().subscribe(	
			(res) => {
	...
	// ============================================
	fillChartByParam(dateDebut, dateFin, chartType) {
		const _this = this;
		this.service.getAllTypeAoBetweenPeriodOrderedByTypeAo(dateDebut, dateFin).subscribe(
				(res) => {
				    let libelle = res.map((data) => data.libelle);
	   	  			let value = res.map((data) => data.number);
	...
	selectionChanged(event: MatRadioChange) {
		...
		if (event.value == "pie" || event.value == "doughnut") {
			this.fillChartByParam(dateDebut, dateFin, event.value);
	...
	public getAOByPeriod(dateDebut, dateFin) {		
		const _this = this;
		this.service
			.getMarchePMEPeriod(dateDebut, dateFin)
			.subscribe(
				(data) => {					
					console.log(data);
					_this.sizeData = data.length;
