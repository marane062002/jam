...
@Component({
  selector: 'kt-add-vignette',
  templateUrl: './add-vignette.component.html',
  styleUrls: ['./add-vignette.component.scss']
})
export class AddVignetteComponent implements OnInit {
  ...
  newVignette(type:string) {
    console.log(type)
     if(type=="v"){
      let v= this.fb.group({
        first: new FormControl("",[Validators.required,Validators.minLength(3), Validators.maxLength(16)]),
        latest:  new FormControl("",[Validators.required,Validators.minLength(3), Validators.maxLength(16)]),
      })
      this.getFormVignettes().push(v);
    }else{ 
      let v= this.fb.group({
        first: new FormControl("",[Validators.required,Validators.minLength(3), Validators.maxLength(16),Validators.pattern("^C[0-9]{7}[A-Z]{2}[0-9]{6}$")]),
...
onSubmit(){
  ...
  let value = this.FormVignette.value
  console.log(value);
