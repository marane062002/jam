...
export class ReintegrationComponent implements OnInit {
  ...
  headerData:any=[
    { name: "N° magasin", content: "magasin.numeroMagasin" },
    { name: "Ref.B.S ", content: "sortie.demandeFourniture.numeroDemande" },
    { name: "Code Analytique", content: "codeAnalytique" },
    { name: "Date", content: "date" },
    //{ name: "Statut", content: "etat" },
  ]
 // fonctionnaire : FonctionnaireDTO = this.localStorege.retrieve("userInfo") as  FonctionnaireDTO ;
  ...
  selectArticle(event:any){
    let id=event.target.value;
   this.quanitieLivre= this.listDemandesFourniture.find(e=>e.article.id==id).quantiteLivre;
  }
