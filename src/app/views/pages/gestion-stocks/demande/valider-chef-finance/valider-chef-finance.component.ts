...
@Component({
  selector: 'app-valider-chef-finance',
  templateUrl: './valider-chef-finance.component.html',
  styleUrls: ['./valider-chef-finance.component.scss']
})
export class ValiderChefFinanceComponent implements OnInit {
   ...
   try {
    ...
    validerDemandeFourniture() : void {
      try {
        ...
        this.demandeFournitureService.validerParChefFinance(validerDemandeFourniture).subscribe((res: DemandeFournitureStringResponse) => {
          this.modelValidation();
        })
      } catch (error) {
        throwError(error)
