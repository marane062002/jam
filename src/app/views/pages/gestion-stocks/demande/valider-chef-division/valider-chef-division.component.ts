...
@Component({
  selector: 'app-valider-chef-division',
  templateUrl: './valider-chef-division.component.html',
  styleUrls: ['./valider-chef-division.component.scss']
})
export class ValiderChefDivisionComponent implements OnInit {
   ...
   try {
    ...
    validerDemandeFourniture() : void {
      try {
        ...
        this.demandeFournitureService.validerParChefDevision(validerDemandeFourniture).subscribe((res: DemandeFournitureStringResponse) => {
          this.modelValidation();
        })
      } catch (error) {
        throwError(error)
