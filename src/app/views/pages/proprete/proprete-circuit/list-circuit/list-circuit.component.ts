...
export class ListCircuitComponent implements OnInit {
	...
	deleteEquipement(id): void {
		this.httpClient.delete(`${this.baseUrl}circuit/${id}`,{ headers: this.headers }).subscribe(
			(response) => {
	...
	ngOnInit() {
		this.columns = ["DENSITE", "DESIGNATION","QUARTIER","ZONE"];
	...
	loadData(page: number, pageSize: number): void {
		...
		this.httpClient.get<any[]>(`${this.baseUrl}circuit/paginate/${page}/${pageSize}`,{ headers: this.headers } ).subscribe((response: any) => {
	...
	addCercuit(): void {
		this.router.navigate(["/pages/proprete-circuit/add-circuit"]);
	}

	Carte(): void {
		this.router.navigate(["/pages/proprete-circuit/evaluation-circuits"]);
	}
	...
	Modifier(id): void {
		this.router.navigate([`/pages/proprete-circuit/edit-circuit/${id}`]);
	}
}
