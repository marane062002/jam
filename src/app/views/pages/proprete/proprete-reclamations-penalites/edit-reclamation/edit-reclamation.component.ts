...
export class EditReclamationComponent implements OnInit {
	...
	ngOnInit() {
		...
		this.reclamationForm = this.formBuilder.group({
		  ...
		  this.httpClient.get<any[]>(`${this.baseUrl}reclamation/${this.id}`,{ headers: this.headers } ).subscribe((res: any) => {
			this.reclamationForm.patchValue({ ...res });
	...
	async getAllPjImm(ide) {
        await this.httpClient.get(`${this.AlfresscoURL}/reclamation-proprete/index/${ide}`)
		.subscribe(
            (data:any) => {
				// 
	...
	onSubmit(){
		this.httpClient.put<any[]>(`${this.baseUrl}reclamation/${this.id}`,this.reclamationForm.value,{ headers: this.headers }).subscribe(
			(res:any) => {
