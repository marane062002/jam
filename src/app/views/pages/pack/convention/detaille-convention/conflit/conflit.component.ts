...
import { AddConflitComponent } from './add-conflit/add-conflit.component';
...
@Component({
	...
	styleUrls: ['./conflit.component.scss']
})
export class ConflitComponent implements OnInit {
	...
	constructor(
		...
		public conflitMarcheService: ConflitMarcheService,
	...
	ngOnInit() {
		...
		this.conflitMarcheService.all(this.id).subscribe(res => {
			this.dataShow = res;
	...
	openDialog(id): void {
		/* const dialogRef = this.dialog.open( AddConflitComponent,{
		 width: '800px',
		 data: {
			   id: this.id,
			   conflit_id:id
		...
		if (id != 0) {
			this.Ajouter = true;
			this.conflitMarcheService.findById(id).subscribe(res => {
	...
	delete(id: number): void {
		Swal.fire({
			...
			if (result.isConfirmed) {
				this.conflitMarcheService.delete(id).subscribe(res => {
					Swal.fire({
						position: 'center',
						icon: 'success',
	...
	annuler() {
		this.Ajouter = false;
		this.formAutorisation.reset();
	}


	onSubmit() {
		this.formAutorisation.value.convention.id = parseInt(this.idConvention);
