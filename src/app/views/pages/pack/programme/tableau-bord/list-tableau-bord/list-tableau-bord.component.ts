...
import { ExcelAssociationService } from "../../../../../../../app/views/pages/utils/excel-association.service";

@Component({
	selector: "kt-list-tableau-bord",
	templateUrl: "./list-tableau-bord.component.html",
	styleUrls: ["./list-tableau-bord.component.scss"],
})
export class ListTableauBordComponent implements OnInit {
...
listeConventionObject
	ngOnInit() {
		...
		this.route.queryParams.subscribe(params => {
			...
			const id=params['id'];
			// Use this updatedProgramId to highlight or filter the program
		});
		let e={
...
this.handlePageEvent(e);
		...
		this.tableauBordService.findAllByPages().then(
			(res: any) => {
	...
	addNew(): void {
		this.router.navigate(["/programme/add-tableau-bord"], {
		});
	...
	delete(id: number): void {
		Swal.fire({
			...
			if (result.isConfirmed) {
				this.tableauBordService.delete(id).subscribe(
					(res) => {
						Swal.fire({
							position: "center",
							icon: "success",
	...
	handlePageEvent(event: PageEvent) {
		...
		this.tableauBordService.Pagination(this.pageIndex,this.pageSize).subscribe((data:any)=>{
...
if(this.formGroup.value.convention!=undefined){
	...
	if (this.formGroup.value.nature!=undefined) {
		...
		row.nature = row.nature.map((item) => item.id);
	
		if (localStorage.getItem("language") == "fr") {
			this.isLoading = true;
			
			this.programeServie.research(0, 500, row).subscribe(
				(res: any) => {
					this.isLoading = false;
					console.log(res);
