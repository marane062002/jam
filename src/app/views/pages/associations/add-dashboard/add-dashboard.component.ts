...
export class AddDashboardComponent implements OnInit {
	...
	// constructeur
	// ============================================
	constructor(private service5: SubventionsService,private notification: NotificationService,private spinnerService:SpinnerService, private subventionService:SubventionsService, private service: AssociationService, private service2: OrganisationService, private fb: FormBuilder, private translate: TranslateService, private datePipe: DatePipe) {
		this.getSubventions();
	...
	onSubmit() {
		...
		if (this.searchForm.get("dateFin").value != "" && this.searchForm.get("dateDebut").value == "") {
			...
			if (dateDebut != "" && dateFin != "" && anneeSubvention=="" && arrondissement.length==0) {
				...
				this.subventionService.getSubsByFilterOnlyDate(dateDebut, dateFin).subscribe(
					(data) => {
			...
			if (dateDebut == "" && dateFin == "" && anneeSubvention!="" && arrondissement.length==0) {
				...
				var spinnerRef = this.spinnerService.start(this.translate.instant("PAGES.GENERAL.LOADING")); // start spinner
				this.subventionService.getSubsByFilterAnneeSubv(anneeSubvention).subscribe(
					(data) => {
						this.spinnerService.stop(spinnerRef); // stop spinner
						this.isLoading = false;
						_this.dataSize = data.length;
			...
			if (dateDebut != "" && dateFin != "" && anneeSubvention!="" && arrondissement.length==0) {
				...
				var spinnerRef = this.spinnerService.start(this.translate.instant("PAGES.GENERAL.LOADING")); // start spinner
				this.subventionService.getSubsByFilterOnlyDateAndAnneeSubv(dateDebut, dateFin,anneeSubvention).subscribe(
					(data) => {
						this.spinnerService.stop(spinnerRef); // stop spinner
						this.isLoading = false;
						_this.dataSize = data.length;
			...
			if (dateDebut != "" && dateFin != "" && anneeSubvention=="" && arrondissement.length!=0) {
				...
				var spinnerRef = this.spinnerService.start(this.translate.instant("PAGES.GENERAL.LOADING")); // start spinner
				this.subventionService.getSubsByFilterOnlyDateAndArron(dateDebut, dateFin,arrondissement).subscribe(
					(data) => {
						this.spinnerService.stop(spinnerRef); // stop spinner
						this.isLoading = false;
						_this.dataSize = data.length;
			...
			if (dateDebut != "" && dateFin != "" && anneeSubvention!="" && arrondissement.length!=0) {
				...
				var spinnerRef = this.spinnerService.start(this.translate.instant("PAGES.GENERAL.LOADING")); // start spinner
				this.subventionService.getSubsByFilterOnlyDateAndArronAndAnneeSubv(dateDebut, dateFin,arrondissement,anneeSubvention).subscribe(
					(data) => {
						this.spinnerService.stop(spinnerRef); // stop spinner
						this.isLoading = false;
						_this.dataSize = data.length;
