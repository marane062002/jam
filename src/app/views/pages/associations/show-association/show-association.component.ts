...
export class ShowAssociationComponent implements OnInit {
	...
	// Constructeur
	// ============================================
	constructor(private service: AssociationService, private filesUtil: FilesUtilsService, private router: Router, private route: ActivatedRoute, private translate: TranslateService, private notification: NotificationService, private fileService: FilesUtilsService, private spinnerService: SpinnerService) {
		// Charger la liste des mandats
		this.getMandatBureau();

		//this.getConvention();
		this.asyncTabs = new Observable((observer: Observer<Association360Tab[]>) => {
			setTimeout(() => {
				observer.next([
					// {
					// 	label: "PAGES.ASSOCIATION.TAB_SUBVENTION",
					// 	content: "1",
					// },
					// {
					// 	label: "PAGES.ASSOCIATION.TAB_HEBERGEMENT",
					// 	content: "2",
					// },
					// {
					// 	label: "PAGES.ASSOCIATION.TAB_RESTAURATION",
					// 	content: "3",
					// },
					// {
					// 	label: "PAGES.ASSOCIATION.TAB_IMPRESSION",
					// 	content: "4",
					// },
					{
						label: "PAGES.ASSOCIATION.TAB_DIVERS",
						content: "5",
					},
					// {
	...
	ngOnInit() {
		...
		this.service.getObjectById("/association/show/", this.id).subscribe(
			(data) => {
	...
	deleteMandat(id): void {
		if (confirm(this.translate.instant("PAGES.GENERAL.MSG_DELETED"))) {
			this.service.deleteObject("/mandatBureau/delete/", id).subscribe((data) => {
