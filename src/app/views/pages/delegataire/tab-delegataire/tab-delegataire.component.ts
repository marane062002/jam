...
export class TabDelegataireComponent implements OnInit {
	...
	loadData(page: number, pageSize: number): void {
		...
		this.httpClient.get<any[]>(`${this.baseUrl}suivi-delegataire/delegataire/${this.id}/${page}/${pageSize}`,{ headers: this.headers } ).subscribe((response: any) => {
	...
	addSuiviDelegataire(): void {
		...
		this.router.navigate(['delegataire/add-tab-delegataire'],navigationExtras);
	}
	updateSuiviDelegataire(id: number): void {
		this.router.navigate(['delegataire/upd-tab-delegataire/' + id]);
	}
	...
	deleteDelegataire(id): void {
		this.httpClient.delete(`${this.baseUrl}suivi-delegataire/${id}`,{ headers: this.headers }).subscribe(
			(response) => {
